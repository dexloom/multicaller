...
contract MulticallerCallbackTest is Test, Helper, TestHelper {
    ...
    function testCallErcInsideUni2CallbackArray2() public {
        ...
        bytes memory callDataArray = abi.encodePacked(
            mergeData(address(erc20Mock), callData, ZERO_VALUE_CALL_SELECTOR,EncodeStack(1,0,0x24,0x20) ,0xFFFFFF),
            mergeData(address(erc20Mock), callData, ZERO_VALUE_CALL_SELECTOR,EncodeStack(1,0,0x24,0x20) ,0xFFFFFF)
        ); 
    ...
    function testCallErcInsideUni3CallbackArray() public {
        ...
        bytes memory callData = abi.encodeWithSignature("transfer(address,uint256)", addr,0x1111111111);
        bytes memory callDataArray = abi.encodePacked(
            mergeData(address(erc20Mock), callData, ZERO_VALUE_CALL_SELECTOR,EncodeStack(1,1,0x24,0x20) ,0xFFFFFF)
        );
    ...
    function testCallErcInsideUni2QuickTransfer() public {
        ...
        bytes memory callData = abi.encodeWithSignature("swap(uint256,uint256,address,bytes)", 0x11111, 0x0, address(multicaller), callDoCallsRetToken);

        bytes memory callDataArray = abi.encodePacked(
            mergeData(address(uni2Mock), callData, ZERO_VALUE_CALL_SELECTOR, 0xFFFFFF,0xFFFFFF)
        );
   ...
   function testCallErcInsideUni2QuickTransfer2() public {
        ...
        bytes memory callData = abi.encodeWithSignature("swap(uint256,uint256,address,bytes)", 0x0, 0x11111, address(multicaller), callDoCallsRetToken);
    ...
    function testCallErcInsideUni3QuickTransfer() public {
        ...
        bytes memory callData = abi.encodeWithSignature("swap(address,bool,int256,uint160,bytes)", addr, true, 0x11111, 0x0,  callDoCallsData);
...
function testCallErcInsideUni3CallbackQuickTransfer() public {
        ...
        bytes memory callData2 = abi.encodeWithSignature("swap(address,bool,int256,uint160,bytes)", addr, true, 0x11111, 0x0,  callDoCallsData);
        ...
        bytes memory callDataArray2 = abi.encodePacked(
            mergeData(address(0), vmCode, CALCULATION_CALL_SELECTOR, 0xFFFFFF,0xFFFFFF),
            mergeData(address(uni3Mock), callData, ZERO_VALUE_CALL_SELECTOR, EncodeStack(1, 0, 0x44, 0x20) ,0xFFFFFF)
        );




        bytes memory callData3 = abi.encodeWithSignature("swap(address,bool,int256,uint160,bytes)", addr, true, 0x11111, 0x0,  callDataArray2);
